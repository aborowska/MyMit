...
for p_bar = P_bars
    ...
    % kernel(mu_init)
    if recycle
        [mit1_recycle, summary1] = MitISEM_new_recycle(kernel_init, kernel, mu_init, cont, GamMat);
    else
...
%         if recycle
%             [mit1, summary1] = MitISEM_new_recycle(kernel_init, kernel, mu_init, cont, GamMat);
%         else
%             [mit1, summary1] = MitISEM_new(kernel_init, kernel, mu_init, cont, GamMat);
%         end
        %% QERMit 1b.:
...
eps_hp = eps_hp(ind_real,:);
    ...
    if recycle
        [mit2, summary2] = MitISEM_new_recycle(mit_hl, kernel, mu_init_hl, cont2, GamMat);
    else
