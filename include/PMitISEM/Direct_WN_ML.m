...
loglik = @(sigma2,yy) -0.5*(T*log(2*pi) + T*log(sigma2) + sum(yy.^2)./sigma2);


kernel_init = @(xx) - loglik(xx,y);
tic
[sigma2_mle, Sigma] = fn_initopt(kernel_init, sigma2_init);
time_direct(1,1)= toc;
...
for sim = 1:N_sim
    fprintf('\nDirect sim = %i.\n', sim); 
    y_direct = sqrt(sigma2_mle)*randn(M,H);
...
if save_on
    ...
    save(name,'VaR_direct','ES_direct','sigma2_mle','time_direct')
end
...
tic
[draw_hl, VaR_est, ~, ~] = BigDraw(10000, H, [], p_bar, sigma2_mle, [], y_predict, GamMat);
time_bigdraw = toc;

if save_on
    name = ['results/PMitISEM/',model,'_',algo,'_',num2str(p_bar),'_H',num2str(H),'_VaR_results_Nsim',num2str(N_sim),'.mat'];
    save(name,'VaR_direct','ES_direct','sigma2_mle','time_direct','draw_hl','VaR_est','time_bigdraw')
end