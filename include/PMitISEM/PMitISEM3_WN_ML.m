...
partition = 1:H;
fn_const_X = @(xx) WN_ML_const_X3(xx);
fn_input_X = @(xx) WN_ML_input_X3(xx, sigma2_used);
...
for sim = 1:N_sim 
    fprintf('\nVaR IS iter: %d\n',sim);
    [draw_pmit, lnd_opt, input_X_pmit] = fn_p_rmvgt_dpmit3(M, pmit,  d, SS, partition, fn_const_X, fn_input_X, GamMat);
    y_opt = input_X_pmit.y_cum;
