function Print_PMitISEM_alg_comb(model, horizons, algos, M, N_sim, p_bar,results_path)
    ...
    for est = estimation       
        fname = [results_path,'results_',model,char(est),'_alg_comp.tex'];
        FID = fopen(fname, 'w+');
        ...
        for h = horizons
            ...
            for algo = algos
                try
                    name = [results_path,model,'_',char(algo),char(est),'_',num2str(p_bar),'_H',num2str(h),'_VaR_results_Nsim',num2str(N_sim),'.mat'];
                    load(name)                
                catch
                end

                try
                    name = [results_path,model,'_',char(algo),char(est),'_',num2str(p_bar),'_H',num2str(h),'_RNE_Nsim',num2str(N_sim),'.mat'];               
                    load(name)                    
                catch
                end                
            end
