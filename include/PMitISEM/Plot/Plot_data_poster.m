function data_stats = Plot_data_poster(y,time,save_on,figures_path)
% garch_old: time = [1998,2008]
% gas_crisis: time=[2005, 2016.5]
close all

ff = figure(1);
set(gcf,'units','normalized','outerposition',[0.1 0.1 0.49 0.4]);
...
% subplot(1,3,2)
    ...
    set(h,'FaceColor',[0 0.4470 0.7410], 'EdgeColor','k')
    plotTickLatex2D('FontSize',12);
...
% subplot(1,3,3)
    ...
    text(.0,0.5,descr,'FontSize',14)
    
    
    if save_on
%         name = [figures_path,model,'_hor_direct_H', num2str(H),'.png'];
        name = [figures_path,'data_poster.eps'];
