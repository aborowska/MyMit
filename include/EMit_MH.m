...
% accept - acceptance rate in the independence MH
    ...
    [theta, ~] = fn_rmvgt_robust(N, mit, kernel_prior);
    
    if (N <= 2000)
%         [theta, lnk, ~, x, lng_y, lnw_x, x_smooth] = fn_rmvgt_robust(N, mit, kernel, theta);
        [theta, lnk, ~, x, lng_y, lnw_x] = fn_rmvgt_robust(N, mit, kernel, theta);
    else
        lnk = zeros(N,1);
        x = zeros(N,T); 
        lng_y = zeros(N,1);
...
%             [theta(ind,:), lnk(ind,:), ~, x(ind,:), lng_y(ind,:), lnw_x(ind,:), x_smooth(:,ind)] = fn_rmvgt_robust(1000, mit, kernel, theta(ind,:));      
            [theta(ind,:), lnk(ind,:), ~, x(ind,:), lng_y(ind,:), lnw_x(ind,:)] = fn_rmvgt_robust(1000, mit, kernel, theta(ind,:));      
        end
    end
