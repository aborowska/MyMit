...
cd ../
addpath('include/');
addpath('results/t_garch');


load('t_garch_mitisem_1.mat', 'VaR_IS', 'PL_opt', 'sim') 
VaR_mitisem = VaR_IS(sim,1);
PL_mitisem = PL_opt;

load('t_garch_admit_1.mat', 'VaR_IS', 'PL_opt', 'sim') 
VaR_admit = VaR_IS(sim,1);
PL_admit = PL_opt;

load('t_garch_direct_df1.mat', 'VaR_direct', 'PL_direct', 'sim') 
