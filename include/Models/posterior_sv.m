function  [lnk, x, lng_y, lnw_x, eps_bar, eps_sim, C_T, lnp_T, RND] = posterior_sv(y, theta, par_NAIS_init, prior_const, cont) 
    N = size(theta,1);
    T = size(y,1);
    ...
    for ii = 1:N
        if (mod(ii,100) == 0)
            fprintf('Posterior sv: nais_param ii = %i\n',ii); 
        end
        par_SV = theta(ii,1:3); 
