...
%     fn_lnf = @(x) fn_CoVsq_C_mex(log(x), lnk, lnd); % <-- NEW! MEX FIRST
...
% fn_lnf = @(x)  fn_CoVsq_C(x, lnk, lnd); % fn_lnf = @(x) fn_CoVsq_C(log(x), lnk_up(1:Np,:), lnd_up);
...
%             p_up = fn_optProb(mit.p, lnk=lnk_up(1:Np,:);lnd= lnd_up; cont);
% fn_lnf_p = fn_lnf(p)