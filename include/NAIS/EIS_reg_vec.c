...
void EIS_reg_vec(double *y, double *theta_smooth, double *V_smooth, double *z, double *w, double *tol_C,
                 ...
                 double *b_new, double *C_new)
{
    ...
    for (i=0; i<T; i++)
    {
        for (j=0; j<M; j++)
        {
            ...
            Y[j] = -0.5*(log(2*PI) + theta_GH[j] + y[i]*y[i]/exp(theta_GH[j]));                         
        }
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    /* call the function */
    EIS_reg_vec(y, theta_smooth, V_smooth, z, w, tol_C, T, M, b_new, C_new);
