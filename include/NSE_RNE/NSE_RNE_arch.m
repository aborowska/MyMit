...
% given the VaR value
% and
% evaluate the asymptotically valid normal dnesity p_hat_ES_grid of the ES
% estiamtor on a grid
PL_opt = fn_PL(y_opt);
[PL_opt, ind] = sort(PL_opt); 
w = w_opt(ind,:);
...
for ii = 1:grid_pn
    ...
    dens_curr = struct('y',y_opt(1:ind_curr),'w',w_relevant,'c',ES_grid(ii,1));%,'scale',1);
    p_hat_ES_grid(ii,1) = fn_PL(dens_curr, 2);% the density of PL at ES_grid 
    dens_IS = struct('y',y_opt,'w',w_opt,'c',VaR_curr);%,'scale',1);
    p_hat_VaR_grid(ii,1) = fn_PL(dens_IS, 2); % weights for ES esitmator's density, ie. estiamted density of the VaR estimator
end

%% Step 3: 
% estimate the ES estimator's dnesity p_hat_ES_IS as the weighted
% average of the densities p_hat_ES_grid (given VaR) with weights from the
% estimated dnesity of the VaR estimator p_hat_VaR_grid
...
NSE_ES_IS2 = sqrt(sum((p_hat_ES_grid-p_hat_ES_IS).^2)/grid_pn);
