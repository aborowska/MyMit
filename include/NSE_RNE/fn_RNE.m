...
% For 'MH' input is an optional lag parameter (dafault: the rule of thumb with L= 0.75*N^(1/3))
% For 'IS' input are importacne weights (optinal, default: equal)
% For 'MH' three weighting schemes:  standard Newey-West weights ('NW'), 
% Parzen kernel ('P') or (default) Quadratic kernel ('Q') 
% For 'MH' perfomres prewhitening with the default filter order p=1
    [N,d] = size(theta);
