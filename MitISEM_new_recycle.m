function [mit_new, summary] = MitISEM_new_recycle(kernel_init, kernel, mu_init, cont, GamMat)
    d = size(mu_init,2);
...
%         lnd_new = dmvgt(theta, mit_new, true, GamMat);
%         w = fn_ISwgts(lnk, lnd, norm);
%%%%%%%%%%%
        % UPDATE COMBINED
...
%         [CV_new, hstop_new] = fn_CVstop(w, CV_old, CV_tol);
        [CV_new, hstop_new] = fn_CVstop([], CV_old, CV_tol, lnk, lnd, lnd_new);
        CV = [CV, CV_new]
%         if (H > 1)
            hstop = hstop_new;
%         end       
