function [mit, summary] = AdMit(kernel_init, kernel, mu_init, cont, GamMat)
...
%% Step 0: Initialization
    ...
    % mixture as the mode if the log kernel functiona and minus the Hessian
...
%% Step 1 (cont'd):  
            ...
            [theta_up, lnk_up, ind_red] = fn_rmvgt_robust(Ns, mit_up, kernel, resampl_on);
            display(ind_red);            
        ...
        %%
%         mit_h.mu = mu; % mu is known from the initial optimazation
        mit_h.df = cont.dfnc;
        mit_h.p = 1;
...
%             fprintf('m = %i\n', m);
            ...
            %% form matrix used in optimazation of probabilities
            lnd_up = zeros(Np, H^2);
